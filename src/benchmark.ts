...
async function runBenchmarks() {
    ...
    for (const wavelet of wavelets) {
      for (const size of sizes) {
        ...
        console.info(`Running wasmlets benchmarks for size ${size} and wavelet ${wavelet}`);
        //////////////////////////////////////////////////////////////////////////
        // Wavedec benchmark
        ...
        console.info(`wavedec avg time: ${wasmletsDecAvg}ms (${numDecTrials} trials)`);

        //////////////////////////////////////////////////////////////////////////
        // Pyodide benchmarks
        console.info(`Running Pyodide benchmarks for size ${size} and wavelet ${wavelet}`);
...
json.dumps({
    ...
    'numRecTrials': num_rec_trials
        ...
        `);
        const { pyodideDecAvg, pyodideRecAvg, numDecTrials: pyNumDecTrials, numRecTrials: pyNumRecTrials } = JSON.parse(pyodideResult);
        console.info(`wavedec avg time: ${pyodideDecAvg}ms (${pyNumDecTrials} trials)`);
        console.info(`waverec avg time: ${pyodideRecAvg}ms (${pyNumRecTrials} trials)`);

        //////////////////////////////////////////////////////////////////////////
        // Discrete Wavelets benchmarks
        console.info(`Running discrete-wavelets benchmarks for size ${size} and wavelet ${wavelet}`);
        ...
        while (performance.now() - startDiscreteDec < targetDurationMs) {
            discreteCoeffs = (DiscreteWavelets as any).wavedec(regularArray, wavelet as any, 'symmetric');
            numDiscreteDecTrials++;
        }
        const endDiscreteDec = performance.now();
        const discreteDecAvg = (endDiscreteDec - startDiscreteDec) / numDiscreteDecTrials;
